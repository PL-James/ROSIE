...
- Parsing repositories to extract `@gxp-` annotations ([RFC-001](/ROSIE/specs/rfc-001/))
- Building and validating the trace graph
- Computing deterministic manifest hashes
- Communicating with the SoR via the API contract ([RFC-004](/ROSIE/specs/rfc-004/))
- Enforcing release gates in CI/CD pipelines
- Generating evidence packages ([RFC-003](/ROSIE/specs/rfc-003/))

### 1.2 System of Record Responsibilities (Out of Scope)

The SoR is a separate system that implements [RFC-004](/ROSIE/specs/rfc-004/). The SoR handles:
