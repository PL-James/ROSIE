...
- **Define intent**: Author requirements in Markdown ([RFC-001](/specs/rfc-001/))
- **Tag design**: Annotate implementation logic with `@gxp-id` ([RFC-001](/specs/rfc-001/))
- **Prove execution**: Bind tests to requirements via `@gxp-traces`
- **Assure integrity**: Commit changes through the Integrity Guard ([RFC-002](/specs/rfc-002/))
...
| Validation report       | `gxp-execution.json` | Aggregated evidence from [RFC-003](/specs/rfc-003/)    |
...
- **Handshake**: A cryptographic manifest hash ([RFC-002](/specs/rfc-002/)) ensures the SoR only approves a specific, immutable repo state
...
- **CSA (Computer Software Assurance)**: ROSIE uses AI agents ([RFC-002](/specs/rfc-002/)) to highlight high-risk changes, allowing humans to focus on approvals for critical logic

---

## 6. The "Watchdog" (Self-Validation)

As defined in [RFC-005](/specs/rfc-005/), the framework is self-validating. The tool that generates compliance evidence must prove its own integrity (via recursive self-tests) during every execution, ensuring the evidence has not been tampered with or corrupted by the tool itself.
...
|-----|-------|-------------|
| [RFC-001](/specs/rfc-001/) | Data Standard | Tagging and manifests |
| [RFC-002](/specs/rfc-002/) | Engine Spec | Hard-gates and AI protocols |
| [RFC-003](/specs/rfc-003/) | Evidence Standard | Artifact packaging |
| [RFC-004](/specs/rfc-004/) | API Interface | The SoR connector |
| [RFC-005](/specs/rfc-005/) | TQ Baseline | Self-validation and archetypes |
